...
std::shared_ptr<VisionMethods> Robot::visionMethods;
...
//table = NetworkTable::GetTable("GRIP/aGoalContours");
	...
	//visionMethods.reset(new VisionMethods());
...
void Robot::TeleopPeriodic() {
	...
	Scheduler::GetInstance()->Run();
	    	...
	    	std::cout << "aftervideocapture" << std::endl;
	//
	...
	//    	camera.set(CV_CAP_PROP_FRAME_HEIGHT, 480);
	    	...
	    	std::cout << "after gp" << std::endl;

	    	long frameTime = cvsink.GrabFrame(frame);
	    	if(frameTime != 0) {
	    		gp.GripPipelineThree::Process(frame);
	    	...
	    	std::vector<std::vector<cv::Point> >* filters = gp.GripPipelineThree::GetFilterContoursOutput();

	std::cout << "Contour number :" << filters->size() << std::endl;

	std::vector<double> area = m_table->GetNumberArray("area", llvm::ArrayRef<double>());
	...
	std::cout << "Heights: " << heights.size() << std::endl;


//	std::cout << "Ratio: ";
//	for(unsigned int i=1; i < heights.size(); i++){
//		double ratio = heights[i]/widths[i];
//		std::cout << ratio << " ";
//
...
//		std::cout << "Height:" << *heights.data()<< "  Width: " << *widths.data() << std::endl;
//
//		int heightValues = *heights.data();
//		int widthValues = *widths.data();
//		int contourWidth =  4*heightValues;
//
//
//		if(widthValues < (contourWidth + 5) && widthValues > (contourWidth - 5) ){
...
//		else{
//
//			Robot::drivePID->ArcadeDrive(0.3, 0, 0);
//		}
//
//
//	}
