...
void CameraProcessor::calculateDistanceAndAngleOfTarget(Target& target) {
	...
	double camAngle = 15.0;
	double half_image_height = k_resX / 2;
		double pixel_y = half_image_height - (target.y);
		double y = pixel_y / half_image_height;
		double testDistance = (97 - 35.5)/
				tan((y * 26.8 / 2.0 + camAngle) * M_PI / 180);
